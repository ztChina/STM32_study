...
{	
    ...
    while(1)
    {
		...
		{	
			printf(" KEY_UP down\r\n" );			
			mode=!mode;
            if(mode==0)   CAN1_Mode_Init(CAN_SJW_1TQ,CAN_BS2_8TQ,CAN_BS1_9TQ,100,CAN_MODE_NORMAL); //CAN初始化,波特率20Kbps//CAN1_Mode_Init(CAN_SJW_1TQ,CAN_BS2_8TQ,CAN_BS1_9TQ,4,CAN_MODE_NORMAL);        //回环模式,波特率500Kbps
            else if(mode==1) CAN1_Mode_Init(CAN_SJW_1TQ,CAN_BS2_8TQ,CAN_BS1_9TQ,100,CAN_MODE_LOOPBACK); //CAN初始化,波特率20Kbps//CAN1_Mode_Init(CAN_SJW_1TQ,CAN_BS2_8TQ,CAN_BS1_9TQ,4,CAN_MODE_LOOPBACK);  //回环模式,波特率500Kbps
			CAN_Config();
		...
		key=CAN1_Receive_Msg(canbuf_two);
		if(key < 9)//接收到有数据
		{			
			LCD_Fill(30,270,160,310,WHITE);//清除之前的显示
 			for(i=0;i<key;i++)
			{
					printf("recv:%d",canbuf_two[i]);
