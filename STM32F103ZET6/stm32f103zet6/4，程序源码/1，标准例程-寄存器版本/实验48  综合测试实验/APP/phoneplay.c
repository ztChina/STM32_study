...
"拨号中...","撥號中...","Dialing...",
"来电...","來電...","Calling...",
"未找到GSM模块,请检查!","未找到GSM模塊,請檢查!","GSM Module Err!Please Check!",
"SIM卡异常,请检查!","SIM卡異常,請檢查!","SIM Card Err!Please Check!",
"通话中...","通話中...","Talking...",
}; 
//电话接听/拒绝按钮标题
u8*const phone_btncaption_tbl[2][GUI_LANGUAGE_NUM]=
{
"接听","接聽","ANSWER",
"拒绝","拒絕","REFUSE", 
};
...
void phone_incall_task(void *pdata)
{
	...
	{ 
		//窗口的名字和背景色
		strcpy((char*)pbuf,(char*)sim900dev.incallnum);//拷贝号码到pbuf里面
		...
		rbtn->caption=(u8*)phone_btncaption_tbl[1][gui_phy.language];	//标题为拒绝
 		okbtn->caption=(u8*)phone_btncaption_tbl[0][gui_phy.language];	//标题为接听
				...
				if(res&&((p_btn[i]->sta&(1<<7))==0)&&(p_btn[i]->sta&(1<<6)))//有按键按下且松开,并且TP松开了
				{
					...
					if(i==12&&pdis->plen)//仅在有号码的时候进行拨号
					{
						if(sim900dev.mode==0)
						{
							...
							{
 								if((sim900dev.status&0X80)==0)window_msg_box((lcddev.width-180)/2,(lcddev.height-70)/2,180,70,(u8*)phone_remind_tbl[2][gui_phy.language],(u8*)APP_REMIND_CAPTION_TBL[gui_phy.language],12,0,1<<6,1200);
								else window_msg_box((lcddev.width-180)/2,(lcddev.height-70)/2,180,70,(u8*)phone_remind_tbl[3][gui_phy.language],(u8*)APP_REMIND_CAPTION_TBL[gui_phy.language],12,0,1<<6,1200);
 								break;
							} 
