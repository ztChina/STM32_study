...
{
{"ALIENTEK精英V1","ALIENTEK精英V1","ALIENTEK Elite V1",},
...
void sysset_system_status(u16 x,u16 y,u8* caption)
{
 	...
 	while(rval==0)
	{
		...
		if(i==0)//时间到了,更新内存,CPU等的状态
		{
			...
			//显示温度值
			temperate=Get_Temprate();//得到内部温度
			app_show_float(x+200-10-12,y+32+5,temperate/10,1,5,12,SYSSET_INWIN_FONT_COLOR,twin->windowbkc);//显示温度
  			i=100;
		}
		i--;
	}
  	...
  	if(twin!=NULL&&rbtn!=NULL)
	{
		...
		btn_draw(rbtn);	  		//画按键
 		...
 		gui_draw_rectangle(x+10-4,y+32+120+18,188,42,0X4A49);		//画边框
  		...
  		gui_phy.back_color=APP_WIN_BACK_COLOR;
		gui_show_ptstr(x+10,y+32+10,x+10+180,y+32+10+16,0,BLACK,16,(u8*)sysset_system_about_caption_tbl[0][gui_phy.language],0);//ALIENTEK Elite
