...
//APP正在读取提示语
u8*const exeplay_readmsg_tbl[GUI_LANGUAGE_NUM]=
{							 
...
{ 
	...
	if(BKP->DR2==0X5050)				//检查BKP2,如果为0X5050,则说明需要执行app代码
	{
		...
		while(n)						//复制数据 
		{
			W25QXX_Read(xdes+offset,EXEPLAY_SRC_BASE+offset,1024); 
			offset+=1024;	
			n-=1024;
		}
   	...
   	while(rval==0)
	{
		...
		{	    
			...
			if(pname==NULL)rval=1;//申请失败
			{
				...
				if(rval)break;	//打开失败   
				if(f_exe->fsize<EXEPLAY_APP_SIZE)	//可以放的下,并且用户确定执行
				{   
					pbuf=gui_memin_malloc(4096);	//申请4K字节内存
					if(!pbuf)break;					//申请失败
					rval=f_read(f_exe,pbuf,512,(UINT*)&br);//读出前512字节
					if(rval)break;					//读取出错 					
					if(((*(vu32*)(pbuf+4))&0xFF000000)==0x20000000)//判断是否为0X20XXXXXX.检测APP的合法性。
					{
						...
						if(res==1)	//按下了确认键
						{
							window_msg_box((lcddev.width-200)/2,(lcddev.height-100)/2,200,100,(u8*)exeplay_readmsg_tbl[gui_phy.language],(u8*)APP_REMIND_CAPTION_TBL[gui_phy.language],12,0,0,0);//提示正在读取
							f_lseek(f_exe,0);				//偏移到开头
							while(1)						//读取整个文件
							{
								rval=f_read(f_exe,pbuf,4096,(UINT*)&br);//读出BIN的所有内容  
								W25QXX_Write(pbuf,EXEPLAY_SRC_BASE+offset,br);
								offset+=br;
								if(br!=4096)break;
							}
							if(rval)break;	//读取出错 
