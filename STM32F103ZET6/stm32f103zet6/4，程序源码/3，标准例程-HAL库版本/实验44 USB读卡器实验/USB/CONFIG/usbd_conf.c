...
void HAL_PCD_MspInit(PCD_HandleTypeDef *pcdHandle)
{
  ...
  {
    /* Peripheral clock enable 使能USB时钟*/
    __HAL_RCC_USB_CLK_ENABLE();

    /* Peripheral interrupt init 中断初始化 */
...
void HAL_PCD_MspDeInit(PCD_HandleTypeDef *pcdHandle)
{
  if (pcdHandle->Instance == USB)
  {
    __HAL_RCC_USB_CLK_DISABLE(); //失能USB时钟

    /* Peripheral interrupt Deinit*/
    HAL_NVIC_DisableIRQ(USB_LP_CAN1_RX0_IRQn); //失能中断
  }
}
...
//USBD 挂起回调函数
//当低功耗模式启动时debug不能被使用（IAR,Keil不支持）
//hpcd:PCD结构体指针
//返回值:无
...
USBD_StatusTypeDef USBD_LL_Init(USBD_HandleTypeDef *pdev)
{
  ...
  hpcd_USB_FS.Init.battery_charging_enable = DISABLE; //不使能电池充电模式
  if (HAL_PCD_Init(&hpcd_USB_FS) != HAL_OK)           //如果配置不成功则进Error_Handler
  {
    Error_Handler();
  }
